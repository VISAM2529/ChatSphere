...
useEffect(() => {
    const fetchaData1 = async () => {
        try {
            ...
            const response = await axios.get(`https://chatsphere-p6ly.onrender.com/chatInfo/${id.username}/${id.friend}`);
    ...
    const fetchaData2 = async () => {
        try {
            const response = await axios.get(`https://chatsphere-p6ly.onrender.com/FriendData/${id.username}/${id.friend}`);
            if (response.data.length > 0) {
  ...
  fetchaData1()
    ...
    const handleReceivedMessage = async (msg) => {
        try {
            const res = await axios.post("https://chatsphere-p6ly.onrender.com/chat", {
                from: id.username,
                to: id.friend,
